...
During autoplay, the next URL can arrive while the player bar still names the previous song, and the media may restart before the bar catches up. The extension keeps this handoff pending, remembers the rewind even if the title arrives late, and uses the restarted media clock while the old player clock is still visible. A same-title repeat continues from its new position while the new URL settles. Atlas logs the starting position and video ID on each actual track change to help diagnose a real-device mismatch.
...
- **Volume jumps on track change:** verify the Atlas log says `Extension connected: 1.0.7`; disable old unpacked Lyricsflow copies in Edge and reload the YouTube Music tab. The Galaxy slider now adjusts Windows master output, which is independent of YouTube Music's own volume.
