...
The extension reads `HTMLMediaElement.currentTime`, `paused`, `ended`, `playbackRate`, `volume` and `muted`; it listens for `volumechange` on the selected playback element. Unknown volume is `null` until real media volume is observed. Temporary media gaps retain the last observed volume; a newly created media element that defaults to 100% inherits the prior lower volume and mute state before playback. Resets of an existing element to 100% are also restored to the known lower level unless a real desktop or Galaxy gesture **explicitly selects 100%**. Selecting 50% cannot authorize a later reset to 100%. Galaxy commands also update YouTube Music’s own player volume in its page context so its saved level carries across songs. New media elements are set to the known lower volume before playback when possible. Active media is preferred over transient preload elements. A validated `SET_VOLUME` command travels from Galaxy through Atlas to the extension, which clamps the value to 0–1, sets `media.volume`, unmutes on a positive value, and reports the resulting media state. Tablet state follows this report, including desktop volume changes. For diagnostic command and media replacement logging, build with `DEBUG_VOLUME=true npm run build`, recopy/reload the extension, and inspect the YouTube Music tab console. Normal builds do not log volume events. Player bar metadata supplies title, artist, album and cover. Dedicated artist links take precedence, and bullet-separated bylines are parsed as distinct artist/album/year fields. Track duration comes from the player bar clock or progress maximum, confirmed after it settles; a streaming media element may report a growing duration that is unsafe for lyric matching. While duration is unknown, the song and artist still appear and Atlas searches with strict title and full-artist checks, rejecting candidates when distinct recording lengths remain ambiguous. An incomplete read during a song transition does not immediately erase a valid track. Autoplay holds a new URL or title until the new media clock starts, so outgoing playback time is not paired with incoming lyrics. Atlas sends the entire lyric timeline on a track update and sends authoritative playback position. The tablet extrapolates from **local elapsed time since receipt**, schedules the next line boundary, and gently corrects small heartbeat drift. Seek, pause, resume and track changes update immediately. Without synchronized lyrics, it shows track information even if artwork fails.
...
- **Artwork visible but lyrics missing:** check Atlas logs for quoted title, artist and duration. Temporarily launch Atlas with `DEBUG_LYRICS=true` to inspect result counts and candidate rejection reasons. Older missing-result cache entries are ignored by this version; you do not need to clear `data/`. The Atlas log identifies the connected extension build; this update reports `1.0.5`.
...
- **Volume jumps on track change:** verify the Atlas log says `Extension connected: 1.0.5`; disable old unpacked Lyricsflow copies in Edge and reload the YouTube Music tab. An `unknown build` connection means an earlier extension is still running.
