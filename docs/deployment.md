...
Build `1.0.9` updates fast lyric line timing on the Galaxy and sends one playback clock correction per second from the extension. Pull and rebuild Atlas, copy the new `dist/extension` folder onto Windows, reload the extension and YouTube Music tab, and refresh the Galaxy page to load the new display script and styles. The Atlas log should say `Extension connected: 1.0.9`. The Windows master-volume companion and `data/` cache are unchanged.

Build `1.0.10` fills in fractional playback time between YouTube Music player-bar clock ticks even when the HTML media timeline differs. Pull and build on Atlas, copy `dist/extension` to Windows, reload the unpacked extension and YouTube Music tab, and check Atlas logs for `Extension connected: 1.0.10`. If a fresh unpacked folder changes the extension ID, open its Options and set `ws://192.168.1.14:8777/ws` again; its default port is 8766. The Galaxy page and Windows master-volume companion are unchanged by this build.
